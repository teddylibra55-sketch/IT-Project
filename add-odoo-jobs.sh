...
# Script to add Odoo Developer jobs to your IT Jobs website

echo "🚀 Adding Odoo Developer jobs to your IT Jobs website..."
...
if [ "$TOKEN" != "null" ] && [ "$TOKEN" != "" ]; then
    echo "✅ Successfully authenticated! Adding Odoo Developer jobs..."
    
    # Array of Odoo Developer jobs
    jobs=(
        '{
    ...
    # Post each job
    for i in "${!jobs[@]}"; do
        echo "📝 Posting Odoo Developer job $((i+1))..."
        ...
        if [ $? -eq 0 ]; then
            echo "✅ Odoo Developer job $((i+1)) posted successfully!"
        else
    ...
    echo ""
    echo "🎉 All Odoo Developer jobs have been posted successfully!"
    echo ""
    echo "You can now:"
    echo "1. Visit http://localhost:3000 to see all the jobs"
    echo "2. Search for 'Odoo' to filter these jobs"
    echo "3. Filter by location 'Lahore' or 'Pakistan'"
    echo "4. Filter by job type 'remote' or 'onsite'"
    
else
    echo "❌ Failed to authenticate. Make sure the server is running on port 5000"
fi
